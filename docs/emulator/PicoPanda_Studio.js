...
// include: /opt/homebrew/Cellar/emscripten/5.0.6/libexec/src/emrun_prejs.js
/**
 * @license
...
// end include: /opt/homebrew/Cellar/emscripten/5.0.6/libexec/src/emrun_prejs.js
...
function findWasmBinary() {
  return locateFile('PicoPanda_Studio.wasm');
}
...
var ASM_CONSTS = {
 ...
 319258: ($0) => { if (Module['canvas']) { Module['canvas'].style['cursor'] = UTF8ToString($0); } },  
 319341: () => { if (Module['canvas']) { Module['canvas'].style['cursor'] = 'none'; } },  
 319410: () => { return window.innerWidth; },  
 319440: () => { return window.innerHeight; }
};
function pp_wasm_pick_file_to_memfs(target_path_c,accept_c) { const targetPath = UTF8ToString(target_path_c); const accept = UTF8ToString(accept_c); const input = document.createElement('input'); input.type = 'file'; input.accept = (accept && accept.length) ? accept : '*/*'; input.onchange = (e) => { const file = e.target.files && e.target.files[0]; if (!file) return; const reader = new FileReader(); reader.onload = () => { try { const data = new Uint8Array(reader.result); if (typeof FS !== 'undefined' && FS.writeFile) { FS.writeFile(targetPath, data); console.log('WASM import wrote', targetPath, data.length, 'bytes'); } else { console.error('FS.writeFile unavailable'); } } catch (err) { console.error('WASM import failed', err); } }; reader.readAsArrayBuffer(file); }; input.click(); }
function pp_wasm_download_memfs_file(path_c,name_c) { const path = UTF8ToString(path_c); const filename = UTF8ToString(name_c); try { if (typeof FS === 'undefined' || !FS.readFile) { console.error('FS.readFile unavailable'); return; } const data = FS.readFile(path, { encoding: 'binary' }); const blob = new Blob([data], { type: 'application/octet-stream' }); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = filename || 'download.bin'; document.body.appendChild(a); a.click(); a.remove(); setTimeout(() => URL.revokeObjectURL(url), 2000); } catch (err) { console.error('WASM download failed', path, err); } }
function pp_wasm_pick_file_record_path(accept_c) { const accept = UTF8ToString(accept_c); const input = document.createElement('input'); input.type = 'file'; input.accept = (accept && accept.length) ? accept : '*/*'; input.onchange = (e) => { const file = e.target.files && e.target.files[0]; if (!file) return; const reader = new FileReader(); reader.onload = () => { try { const data = new Uint8Array(reader.result); const safeName = (file.name || 'input.bin').replace(/[^a-zA-Z0-9._-]/g, '_'); const path = '/picked_' + safeName; if (typeof FS !== 'undefined' && FS.writeFile) { FS.writeFile(path, data); Module.__ppLastPickedPath = path; } } catch (err) { console.error('WASM import failed', err); } }; reader.readAsArrayBuffer(file); }; input.click(); }
function pp_wasm_consume_last_picked_path(out_c,out_size) { const p = Module.__ppLastPickedPath; if (!p) return 0; stringToUTF8(p, out_c, out_size); Module.__ppLastPickedPath = null; return 1; }
function ImGui_ImplSDL2_EmscriptenOpenURL(url) { url = url ? UTF8ToString(url) : null; if (url) window.open(url, '_blank'); }
function pp_wasm_transfer_is_supported() { if (typeof navigator === 'undefined') return 0; return ('serial' in navigator) ? 1 : 0; }
function pp_wasm_transfer_needs_secure_context() { if (typeof window === 'undefined') return 0; return window.isSecureContext ? 0 : 1; }
function pp_wasm_transfer_refresh_devices() { if (typeof globalThis.PicoPandaWebSerial !== 'undefined') { globalThis.PicoPandaWebSerial.refreshDevices(); } }
function pp_wasm_transfer_request_device() { if (typeof globalThis.PicoPandaWebSerial !== 'undefined') { globalThis.PicoPandaWebSerial.requestDevice(); } }
function pp_wasm_transfer_get_device_count() { if (typeof globalThis.PicoPandaWebSerial === 'undefined') return 0; return globalThis.PicoPandaWebSerial.getDeviceCount(); }
function pp_wasm_transfer_get_device_label(index,out_c,out_size) { if (typeof globalThis.PicoPandaWebSerial === 'undefined') return; const label = globalThis.PicoPandaWebSerial.getDeviceLabel(index) || ''; stringToUTF8(label, out_c, out_size); }
function pp_wasm_transfer_select_device(index) { if (typeof globalThis.PicoPandaWebSerial !== 'undefined') { globalThis.PicoPandaWebSerial.selectDevice(index); } }
function pp_wasm_transfer_start_upload(slot,data,length) { if (typeof globalThis.PicoPandaWebSerial === 'undefined') return 0; return globalThis.PicoPandaWebSerial.startUpload(slot, data, length) ? 1 : 0; }
function pp_wasm_transfer_poll() { if (typeof globalThis.PicoPandaWebSerial !== 'undefined') { globalThis.PicoPandaWebSerial.poll(); } }
function pp_wasm_transfer_copy_status(phase_out,progress_out,status_out,status_size,error_out,error_size,busy_out) { const st = (typeof Module !== 'undefined' && Module._ppTransferStatus) ? Module._ppTransferStatus : { phase: 0, progress: 0, status: 'Ready', error: '', busy: false }; if (phase_out) HEAP32[phase_out >> 2] = st.phase || 0; if (progress_out) HEAPF32[progress_out >> 2] = st.progress || 0; if (status_out) stringToUTF8(st.status || '', status_out, status_size); if (error_out) stringToUTF8(st.error || '', error_out, error_size); if (busy_out) HEAP32[busy_out >> 2] = st.busy ? 1 : 0; }

// Imports from the Wasm binary.
var _wasm_stage_drop_from_heap = Module['_wasm_stage_drop_from_heap'] = makeInvalidEarlyAccess('_wasm_stage_drop_from_heap');
...
function assignWasmExports(wasmExports) {
  assert(typeof wasmExports['wasm_stage_drop_from_heap'] != 'undefined', 'missing Wasm export: wasm_stage_drop_from_heap');
  ...
  _wasm_stage_drop_from_heap = Module['_wasm_stage_drop_from_heap'] = createExportWrapper('wasm_stage_drop_from_heap', 2);
...
var wasmImports = {
  ...
  invoke_vii,
  /** @export */
  pp_wasm_consume_last_picked_path,
  /** @export */
  pp_wasm_download_memfs_file,
  /** @export */
  pp_wasm_pick_file_record_path,
  /** @export */
  pp_wasm_pick_file_to_memfs,
  /** @export */
  pp_wasm_transfer_copy_status,
  /** @export */
  pp_wasm_transfer_get_device_count,
  /** @export */
  pp_wasm_transfer_get_device_label,
  /** @export */
  pp_wasm_transfer_is_supported,
  /** @export */
  pp_wasm_transfer_needs_secure_context,
  /** @export */
  pp_wasm_transfer_poll,
  /** @export */
  pp_wasm_transfer_refresh_devices,
  /** @export */
  pp_wasm_transfer_request_device,
  /** @export */
  pp_wasm_transfer_select_device,
  /** @export */
  pp_wasm_transfer_start_upload
};
...
// include: /opt/homebrew/Cellar/emscripten/5.0.6/libexec/src/emrun_postjs.js
/**
 * @license
 * Copyright 2013 The Emscripten Authors
 * SPDX-License-Identifier: MIT
...
// end include: /opt/homebrew/Cellar/emscripten/5.0.6/libexec/src/emrun_postjs.js
